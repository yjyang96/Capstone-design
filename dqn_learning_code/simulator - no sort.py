...
class Task:
    def __init__(self, debug_flag=False, test_flag=False, state_blink=True, state_inaccurate=True):
        ...
        self.action_space = np.array(range(6))
        # ROS
    ...
    def draw_state_gray(self):
        ...
        gray_color = {"red_ball":255, "blue_ball":50, "wall":100, "robot_red":200, "robot_padding":150,  "robot_roller":175}
        self.frame_gray = np.zeros((simulator["height"]*debug_scale_gray,simulator["width"]*debug_scale_gray,1), np.uint8)
        ...
        cv2.rectangle(self.frame_gray,(simulator["center"]*debug_scale_gray,(simulator["height"]-(Back_pixels+0))*debug_scale_gray+1),\
                     ((simulator["center"]+1)*debug_scale_gray,(simulator["height"]-(Back_pixels-1))*debug_scale_gray-1),gray_color["robot_red"],-1)
                ...
                if cy>=0 and cy < reward_region_y and r_ball[1] >= int(ball_blind_ratio*(abs(1.0*r_ball[0])-ball_blind_bias)-2) and (cx in reward_region_x):
                    if cy >= reward_region_y/3  and cy < 2*reward_region_y/3:
                        if cx == reward_region_x[1]:
                            ...
                            reward += -2
                        if len(self.red_balls_prev) > 0 and int(round(1.0*self.red_balls_prev[i][1]/trans_scale)) < reward_region_y:
                            reward += -5
                    else:
                        if cx == reward_region_x[1]:
                            reward += 8
                ...
                if cy>=0 and cy < reward_region_y and b_ball[1] >= int(ball_blind_ratio*(abs(1.0*b_ball[0])-ball_blind_bias)-2) and (cx in reward_region_x):
                    if cy >=reward_region_y/3  and cy < 2*reward_region_y/3:
                        if cx == reward_region_x[1]:
                            ...
                            reward += -2
                        if len(self.blue_balls_prev) > 0 and int(round(1.0*self.blue_balls_prev[i][1]/trans_scale)) < reward_region_y:
                            reward += -5
                    else: 
                        if cx == reward_region_x[1]:
                            reward += 8
